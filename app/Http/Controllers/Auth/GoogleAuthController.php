...
class GoogleAuthController extends Controller
{
    ...
    public function callback(): RedirectResponse
    {
        ...
        $providerName = trim((string) $googleUser->getName());
        $user->forceFill(['name' => $providerName !== '' ? mb_substr($providerName, 0, 255) : $user->name, 'google_id' => $googleUser->getId(), 'google_email' => $email, 'google_access_token' => $googleUser->token, 'google_refresh_token' => $googleUser->refreshToken ?: $user->google_refresh_token, 'google_token_expires_at' => $googleUser->expiresIn ? now()->addSeconds($googleUser->expiresIn) : null, 'avatar_url' => $user->avatar_url ?: $googleUser->getAvatar(), 'email_verified_at' => $user->email_verified_at ?: now()])->save();

        Auth::login($user, remember: true);
        app(\App\Domain\Audit\AuditRecorder::class)->record('user.login',$user,$user,[],['provider'=>'google']);
